...
CAMP_MAP = {
    ...
    'Nashik-pmax-Used-car-16june':      {'city': 'Nashik',     'platform': 'Google'},
    'UC-DSA-Nashik-UsedCars':           {'city': 'Nashik',     'platform': 'Google'},
    'Nashik-DSA-UsedCars-Discovery':    {'city': 'Nashik',     'platform': 'Google'},
}

CAMP_ALIASES = {
    'Nasik-Search-Buy-UsedCar-8april': 'Nashik-Search-Buy-UsedCar-8april',
}
...
def get_month(d):
    return MONTHS[d.month - 1] + "'" + str(d.year)[2:]
...
def build():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        camp = CAMP_ALIASES.get(rc, rc)
        if not camp:
            continue
        city = str(d.get('CITY') or '').strip()
        if city not in ('Ahmedabad', 'Chandigarh', 'Nashik'):
            continue
        ...
        mo = get_month(dt)
        w = wom(ds)
        key = ds + '|' + camp + '|' + city + '|' + mo + '|' + str(w) + '|' + plat
        if key not in merged:
            merged[key] = {
                'dateStr': ds, 'campaign': camp, 'city': city,
                'month': mo, 'wom': w, 'Platform': plat,
                'spends': 0, 'gen_leads': 0, 'triggered': 0, 'dealer_triggers': 0
            }
    ...
    print("Reading " + spends_path.name + "...")
    ...
    for sheet_name, plat_default in [('Ga', 'Google'), ('FB', 'Meta')]:
        ...
        for row in ws2.iter_rows(min_row=2, values_only=True):
            ...
            if not isinstance(raw_date, datetime):
                continue
            camp = CAMP_ALIASES.get(str(raw_camp or '').strip(), str(raw_camp or '').strip())
            if not camp:
                continue
            cm = CAMP_MAP.get(camp)
    ...
    print("Processed " + str(len(data)) + " rows | " + all_dates[0] + " to " + latest)

    print("Updating " + html_path.name + "...")
    ...
    fmt = str(int(parts[2])) + ' ' + mn[int(parts[1])-1] + ' ' + parts[0] if len(parts) == 3 else latest

    print("")
    print("Done! Dashboard updated - data up to " + fmt)
    print("Open index.html in your browser or push to GitHub to publish.")
